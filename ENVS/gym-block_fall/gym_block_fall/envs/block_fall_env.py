...
class block_fallEnv(gym.Env):
    ...
    def __init__(self, hideDisplay=False):
        ...
        if (hideDisplay):
            self.screen = pg.display.set_mode(
                [SCREEN_WIDTH, SCREEN_HEIGHT], flags=pg.HIDDEN)
        else:
            self.screen = pg.display.set_mode(
                [SCREEN_WIDTH, SCREEN_HEIGHT])

        self.action_space = spaces.Discrete(3)
        self.observation_space = spaces.Box(low=0.0, high=float(
            SCREEN_WIDTH), shape=(3,), dtype=np.float32)
        self.reset()
    ...
    def reset(self):
        ...
        self.block = Block(self, random.randrange(
            0, SCREEN_WIDTH), START_Y_POS)
    ...
    def step(self, action):
        ...
        info = {"Block Pos": "x: {}, y: {}, dist: {}, lives: {}, done: {}, reward: {}".format(
            _blockPos[0], _blockPos[1], _distToGoal, self.lives, done, reward)}
...
class Block(pg.sprite.Sprite):
    ...
    def distFromGoal(self):
        ...
        maxDist = math.sqrt(math.pow(goalCenterX, 2) +
                            math.pow(goalCenterY, 2))
        curDist = abs(math.sqrt(math.pow(goalCenterX - self.x,
                      2) + math.pow(goalCenterY - self.y, 2)))
